...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        while(m_left != 0.0 and m_right == 0.0):
            left_roi_img = roi_edge_img[340: 420, 180: 500]
            
            all_lines = get_lines(left_roi_img, 1, math.pi/180, 50, 10, 5)
            if all_lines is None:
                continue
            only_left_lines = all_lines
            
            m_left, b_left = get_left_lines(only_left_lines)
            print("m_left", m_left)

            x_left = calculate_x(m_left, b_left, L_ROW)
            
            x_midpoint = x_left + 80
